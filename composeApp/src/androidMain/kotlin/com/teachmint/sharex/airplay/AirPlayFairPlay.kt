...
/**
 * FairPlay 2.5 fp-setup handler for AirPlay screen mirroring.
 *
 * Two-phase handshake:
 *   M1 (16B in)  → M2 (142B out)  static response selected by mode byte req[14]
 *   M3 (164B in) → M4 (32B out)   FP header + passthrough of req[144..163]
 *
 * Static response tables sourced from RPiPlay (MIT License):
 * https://github.com/FD-/RPiPlay  —  lib/fairplay_playfair.c
 */
object AirPlayFairPlay {

    private const val TAG = "AirPlay/FP"

    /** Stored M3 body (164 bytes) from fp-setup phase 2 — needed for playfair_decrypt key derivation. */
    @Volatile
    var m3Data: ByteArray? = null
        private set

    /** Converts vararg Int values to a signed ByteArray (low 8 bits only). */
    private fun b(vararg v: Int): ByteArray = ByteArray(v.size) { v[it].toByte() }

    /**
     * Four static 142-byte M2 responses, one per mode (req[14] in range 0–3).
     * In the original C source the array is declared char[4][142]; any shortfall
     * in the initialiser is zero-padded by the C compiler — mode 1 gets a trailing 0x00.
     */
    private val REPLY: Array<ByteArray> = arrayOf(
        /* mode 0 */ b(
        ...
        /* mode 1 — source has 141 initialisers; C zero-fills the 142nd byte */ b(
            ...
            0x75, 0xea, 0x5a, 0x44, 0xd9, 0x59, 0x1c, 0x72, 0x56, 0xb9, 0xfb, 0xe6, 0x51, 0x38, 0x98, 0x82,
            0x27, 0x72, 0x19, 0x88, 0x57, 0x16, 0x50, 0x94, 0x2a, 0xd9, 0x46, 0x68, 0x8a, 0x00
        ),
        /* mode 2 */ b(
    ...
    /** 12-byte header prepended to M4 passthrough response. */
    private val FP_HEADER: ByteArray = b(
        0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14
    )

    /**
     * Route an fp-setup request by body size:
     *   16B  → M2 (142B)
     *   164B → M4 (32B)
     *
     * Returns null on any validation failure; caller should respond with 200 + empty body
     * so the session is not hard-rejected.
     */
    fun handle(body: ByteArray): ByteArray? = when (body.size) {
        16   -> setupM1(body)
        164  -> handshakeM3(body)
        else -> { Log.w(TAG, "Unexpected fp-setup body ${body.size}B"); null }
    }

    // ── phase 1: M1 (16B) → M2 (142B) ────────────────────────────────────────

    private fun setupM1(req: ByteArray): ByteArray? {
        if (req[4].toUByte().toInt() != 0x03) {
            Log.w(TAG, "Unsupported FP version 0x${req[4].toUByte().toString(16)}")
            return null
        }
        // New FairPlay handshake started. Clear stale M3 from prior sessions so we
        // never derive keys from an old reconnect.
        m3Data = null
        val mode = req[14].toUByte().toInt()
        if (mode >= REPLY.size) {
            Log.w(TAG, "Unknown fp-setup mode $mode")
            return null
        }
        Log.d(TAG, "fp-setup M1 → mode=$mode, replying ${REPLY[mode].size}B")
        return REPLY[mode].copyOf()
    }

    // ── phase 2: M3 (164B) → M4 (32B) ────────────────────────────────────────

    private fun handshakeM3(req: ByteArray): ByteArray? {
        if (req[4].toUByte().toInt() != 0x03) {
            Log.w(TAG, "Unsupported FP version in M3")
            return null
        }
        // Store M3 data for later key derivation via playfair_decrypt
        m3Data = req.copyOf()
        Log.d(TAG, "fp-setup M3 → stored ${req.size}B for key derivation")
        // M4 = FP_HEADER (12B) + req[144..163] (20B)
        val res = ByteArray(32)
        FP_HEADER.copyInto(res, destinationOffset = 0)
        req.copyInto(res, destinationOffset = 12, startIndex = 144, endIndex = 164)
        Log.d(TAG, "fp-setup M3 → 32B M4")
        return res
    }
}
