...
interface WebRtcEngine {
     ...
     * with an audio pipeline that transmits the device's REAL microphone —
     * BYOM streams the room's camera + mic into the client's meeting app. The
     * screen-share pipeline may deliberately replace mic samples with system
     * playback audio (as Android does), which would make a BYOM mic silent.
     * Default falls back to [createPeerConnection] for platforms whose regular
     * audio path already captures the microphone.
     */
    suspend fun createByomPeerConnection(iceServers: List<IceServerConfig>): WebRtcPeerConnection =
        createPeerConnection(iceServers)

    /**
     * Capture the device's physical microphone for a BYOM session. The returned
     * track must be added only to a connection from [createByomPeerConnection].
     * Returns null when the platform cannot capture the mic (permission denied
     * or unsupported); callers should continue video-only.
     */
    suspend fun startMicrophoneCapture(): PlatformAudioTrack? = null

    /** Stop and release any microphone capture started via [startMicrophoneCapture]. */
